...
function fillEventForm(evt) {
  ...
  document.getElementById('event-primary').value = evt.primary_color || '#ffffff';
  document.getElementById('event-accent').value = evt.accent_color || '#e50000';
  document.getElementById('event-secondary').value = evt.secondary_color || '#0a0a0a';
...
document.getElementById('clear-tickets-btn').addEventListener('click', async () => {
  ...
  try {
    ...
    if (!confirmAction(`Delete ALL ${stats.total} tickets and attendees for ${eventName}? Already-generated PDF tickets will stop scanning. This cannot be undone.`)) return;
...
document.getElementById('import-btn').addEventListener('click', async () => {
  ...
  try {
    const stats = await API.getStats(eventId);
    if (stats.total > 0 && !confirmAction(`This event already has ${stats.total} tickets. Importing will ADD ${parsedAttendees.length} more attendees on top — it will not replace the existing ones. Continue?`)) return;
  } catch (e) {}
